...
def main():
    ...
    if row>0:
        ...
        file_result=open('./result/result_odinary.txt','w')
    ...
    for rownum in xrange(row):
        ...
        #ub=[1,0.5,2*meanNum,1.1,2*maxNum,1.1,2*maxNum]
        ...
        if function=="SSE": 
            ...
            xopt4,fopt4=pso(RMSE,lb,ub,args=args,initialData=initialData,initialData2=initialData2,swarmsize=250,maxiter=iteration,minfunc=1e-08)
            print('Optimal function values:')
    ...
    if row>0:
        file_result.close()
        print("***********************attention************************************")
        print('please go to dir of result to save result_odinary.txt file and result_all_iteration.txt')
