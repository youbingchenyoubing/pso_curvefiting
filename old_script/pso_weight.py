...
def myfunction(parameters,*args):
    ...
    for i in xrange(size):
        ...
        SSE=SSE+math.pow((y[i]-yi),2)*weight[i]
    meanvalue=caculatemean(args,size)
    SST=0.0
    for i in xrange(size):
        SST=SST+math.pow((y[i]-meanvalue),2)*weight[i]
        if SST==0:
...
def main():
    ...
    for rownum in xrange(row):
        ...
        initialData=[minNum,0.01,meanNum,-1,maxNum,0.7,maxNum] 
        initialData2=[minNum,0.03,meanNum,0.2,maxNum,-1,maxNum]
